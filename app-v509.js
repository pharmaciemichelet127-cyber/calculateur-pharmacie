...
// Marches virtuels importes depuis MAP_PRODUIT_MARCHE (persistent en localStorage)
var condMarchesVirtuels = (function () {
  try { var s = localStorage.getItem('cond_marches_virtuels'); return s ? JSON.parse(s) : {}; }
  catch (e) { return {}; }
})();

// Table des libelles lisibles pour les sous-marches connus
var MEQ_LIBELLES = {
...
// Purge manuelle depuis la console : meqResetMarchesVirtuels()
function meqResetMarchesVirtuels() {
  condMarchesVirtuels = {};
  try { localStorage.removeItem('cond_marches_virtuels'); } catch(e) {}
  meqInited = false;
  meqInit();
  meqRender();
  alert('Marchés virtuels réinitialisés. Les données de test sont réactivées.');
}

// Import XLSX MAP_PRODUIT_MARCHE → condMarchesVirtuels
// Format attendu : colonnes SOUS_MARCHE (ou SOUS_MARCHÉ), LABO, QTES_12M
// Colonne optionnelle : STOCK_TOTAL, LIBELLE_SM
function meqImporterXLSX() {
  ...
  input.onchange = function (e) {
    ...
    reader.onload = function (ev) {
      try {
        ...
        // Chercher la feuille pertinente (contient SOUS_MARCHE et LABO)
        var sheetName = null;
        wb.SheetNames.forEach(function (n) {
          if (!sheetName) {
            ...
            if (json.length > 0) {
              var hdrs = json[0].map(function (h) { return String(h).toUpperCase().trim().replace(/[ÉÈÊ]/g,'E').replace(/[À]/g,'A'); });
              if (hdrs.indexOf('SOUS_MARCHE') >= 0 && hdrs.indexOf('LABO') >= 0) sheetName = n;
            }
          }
        });
        if (!sheetName) {
          alert('Aucune feuille avec colonnes SOUS_MARCHE et LABO trouvée.\nVérifiez le fichier exporté depuis MAP_PRODUIT_MARCHE.');
          return;
        }
        var ws = wb.Sheets[sheetName];
        var rows = XLSX.utils.sheet_to_json(ws, { header: 1, defval: '' });
        if (rows.length < 2) { alert('Feuille vide.'); return; }

        // Mapper les colonnes
        var hdrs = rows[0].map(function (h) { return String(h).toUpperCase().trim().replace(/[ÉÈÊ]/g,'E').replace(/[À]/g,'A'); });
        ...
        if (iSM < 0 || iLABO < 0) { alert('Colonnes SOUS_MARCHE et/ou LABO introuvables.'); return; }

        // Construire un dict des nouvelles données
        var nouveau = {};
        rows.slice(1).forEach(function (r) {
          var sm   = String(r[iSM]  || '').trim().toUpperCase();
          var labo = String(r[iLABO]|| '').trim().toUpperCase();
          if (!sm || !labo) return;
          ...
          if (!nouveau[sm]) nouveau[sm] = { libelle: lib, produits: {} };
          if (lib && !nouveau[sm].libelle) nouveau[sm].libelle = lib;
          nouveau[sm].produits[labo] = { qtes_12m: qtes, stock_total: stock };
        });

        // Merge protecteur dans condMarchesVirtuels
        var nbSM = 0, nbLabo = 0, nbVerif = 0;
        Object.keys(nouveau).forEach(function (smId) {
          nbSM++;
          var src = nouveau[smId];
          if (!condMarchesVirtuels[smId]) {
            // Nouveau sous-marché : créer
            condMarchesVirtuels[smId] = {
              libelle: src.libelle || MEQ_LIBELLES[smId] || smId,
              segment: 'SOCLE',
              produits: []
            };
          } else {
            // Sous-marché existant : garder libellé s'il était absent
            if (!condMarchesVirtuels[smId].libelle && (src.libelle || MEQ_LIBELLES[smId])) {
              condMarchesVirtuels[smId].libelle = src.libelle || MEQ_LIBELLES[smId];
            }
            // Flaguer les labos disparus
            condMarchesVirtuels[smId].produits.forEach(function (p) {
              if (!src.produits[p.labo_nom.toUpperCase()]) {
                p.a_verifier = true;
                nbVerif++;
              } else {
                p.a_verifier = false;
          ...
          // Ajouter ou mettre à jour les labos
          Object.keys(src.produits).forEach(function (laboKey) {
            var data = src.produits[laboKey];
            var existing = condMarchesVirtuels[smId].produits.find(function (p) {
              return p.labo_nom.toUpperCase() === laboKey;
            });
            if (existing) {
              existing.qtes_12m    = data.qtes_12m;
              existing.stock_total = data.stock_total;
            } else {
              condMarchesVirtuels[smId].produits.push({
                ean:         '',
                labo_nom:    laboKey,
                designation: laboKey,
                qtes_12m:    data.qtes_12m,
                stock_total: data.stock_total,
                colisage:    1,
                statut_ref:  'à vérifier',
                a_verifier:  false
              });
              nbLabo++;
            }
          });
          // Trier par qtes_12m décroissant
          condMarchesVirtuels[smId].produits.sort(function (a, b) { return (b.qtes_12m||0) - (a.qtes_12m||0); });
          // Ajouter libellé par défaut si encore manquant
          if (!condMarchesVirtuels[smId].libelle) condMarchesVirtuels[smId].libelle = MEQ_LIBELLES[smId] || smId;
        });
        ...
        var msg = '✅ Import MAP_PRODUIT_MARCHE terminé\n\n' +
          '• ' + nbSM + ' sous-marchés traités\n' +
          '• ' + nbLabo + ' nouveaux labos ajoutés\n' +
          (nbVerif ? '• ⚠ ' + nbVerif + ' labos précédents non retrouvés (flaggés "à vérifier")\n' : '') +
          '\nFichier : ' + sheetName;
        alert(msg);
...
// Donnees de demonstration - structure visee pour l'import reel :
...
function meqInit() {
  ...
  sel.innerHTML = data.map(function (sm) {
    return '<option value="' + sm.id + '">' + sm.libelle + ' (' + sm.segment + ')</option>';
  }).join('');
}

function meqGetData() {
  if (typeof condMarchesVirtuels === 'object' && condMarchesVirtuels && Object.keys(condMarchesVirtuels).length) {
    return Object.keys(condMarchesVirtuels)
      .map(function (k) {
        var sm = condMarchesVirtuels[k];
        var totalQtes = (sm.produits || []).reduce(function (s, p) { return s + (p.qtes_12m || 0); }, 0);
        return { id: k, libelle: sm.libelle || MEQ_LIBELLES[k] || k, segment: sm.segment || 'SOCLE', produits: sm.produits || [], totalQtes: totalQtes };
      })
      .sort(function (a, b) { return (b.totalQtes || 0) - (a.totalQtes || 0); });
  }
  return meqDataTest;
}

function meqEstDonneesReelles() {
  return typeof condMarchesVirtuels === 'object' && condMarchesVirtuels && Object.keys(condMarchesVirtuels).length > 0;
}

// Cherche dans condLabos un labo dont le nom correspond (insensible casse/accents simples)
...
// Retourne {remise, marcheLabel, franco} en lisant le palier actif si present,
// sinon la remise de base du premier marche du labo.
function meqConditionsLabo(laboObj) {
  if (!laboObj) return null;
  var marches = laboObj.marches_negocies || [];
  var m = marches.find(function (mm) { return mm.palier_actif; }) || marches[0];
  var remise = m ? (m.rem || 0) : 0;
  var label = m ? (m.label || m.marche_id || '') : '';
  return { remise: remise, marcheLabel: label, franco: laboObj.francoPort || 0 };
}
...
function meqRender() {
  ...
  var badgeSrc = meqEstDonneesReelles()
    ? '<span style="font-size:10px;padding:2px 7px;background:#d4edda;color:#155724;border-radius:4px;margin-left:8px">✓ Données réelles</span>'
    : '<span style="font-size:10px;padding:2px 7px;background:#fff3cd;color:#856404;border-radius:4px;margin-left:8px">⚠ Données de test</span>';

  var rows = sm.produits.map(function (p) {
    var laboObj = meqTrouverLaboParNom(p.labo_nom);
    var cond = meqConditionsLabo(laboObj);
    var part = totalQtes > 0 ? ((p.qtes_12m || 0) / totalQtes * 100).toFixed(0) : '0';
    var estSelectionne = selectionne === p.labo_nom;
    var condHtml = cond
      ? 'Remise <b>' + cond.remise + '%</b>' + (cond.marcheLabel ? ' (' + cond.marcheLabel + ')' : '') + ' · Franco <b>' + cond.franco + '€</b>'
      : '<span style="color:var(--text-ter)">Non dans Conditions commerciales</span>';
    var verifBadge = p.a_verifier
      ? ' <span style="font-size:9px;background:#fff3cd;color:#856404;padding:1px 5px;border-radius:3px">à vérifier</span>'
      : '';
    return '<tr style="' + (estSelectionne ? 'background:var(--accent-bg)' : '') + '">' +
      '<td style="padding:6px 8px;font-weight:600">' + p.labo_nom + verifBadge + '</td>' +
      '<td style="padding:6px 8px;font-size:11px;color:var(--text-sec)">' + (p.designation || p.labo_nom) + (p.ean ? '<br><span style="font-size:10px;color:var(--text-ter)">EAN ' + p.ean + '</span>' : '') + '</td>' +
      '<td style="padding:6px 8px;text-align:right">' + (p.qtes_12m || 0) + '</td>' +
      '<td style="padding:6px 8px;text-align:right">' + part + '%</td>' +
      '<td style="padding:6px 8px">' + (p.statut_ref || '') + '</td>' +
      '<td style="padding:6px 8px;font-size:11px">' + condHtml + '</td>' +
      '<td style="padding:6px 8px;text-align:center">' +
        '<button type="button" onclick="meqChoisir(\'' + sm.id + '\',\'' + p.labo_nom.replace(/\'/g, "\\\'") + '\')" style="font-size:11px;padding:4px 10px;border-radius:var(--radius-sm);border:1px solid var(--accent);background:' + (estSelectionne ? 'var(--accent)' : '#fff') + ';color:' + (estSelectionne ? '#fff' : 'var(--accent-text)') + ';cursor:pointer;white-space:nowrap">' + (estSelectionne ? '✓ Retenu' : 'Retenir') + '</button>' +
      '</td>' +
      '</tr>';
  }).join('');
  ...
  content.innerHTML =
    ...
    '<th style="text-align:left;padding:6px 8px">Produit équivalent</th>' +
    '<th style="text-align:right;padding:6px 8px">Qtés 12M</th>' +
    '<th style="text-align:right;padding:6px 8px">Part marché</th>' +
    '<th style="text-align:left;padding:6px 8px">Statut</th>' +
    '<th style="text-align:left;padding:6px 8px">Conditions live</th>' +
    '<th></th>' +
    '</tr></thead><tbody>' + rows + '</tbody></table>' +
    (selectionne ? '<p style="font-size:11px;color:var(--text-sec);margin-top:8px">Choix retenu pour ce marché : <b>' + selectionne + '</b>. <button type="button" onclick="meqChoisir(\'' + sm.id + '\',null)" style="font-size:10px;padding:2px 6px;border:none;background:none;color:var(--danger);cursor:pointer">annuler</button></p>' : '') +
    '</div>';
}
