...
function simLPPRGammeProduits() {
  var lignes = [];
  var defs = [
    { motCle: 'THUASNE', filtre: function(p){ return p.famille === 'Compression'; } },
    { motCle: 'PODOWELL', filtre: function(p){ return p.famille === 'CHUT' || p.famille === 'CHUP'; } },
    { motCle: 'GIBAUD', filtre: function(p){ return true; } },
    { motCle: 'ENOVIS', filtre: function(p){ return true; } }
  ];
  defs.forEach(function(def) {
    var labo = simLPPRTrouverLabo(def.motCle);
    if (!labo) return;
    (labo.produits || []).forEach(function(p) {
      if ((p.lppr || 0) > 0 && def.filtre(p)) {
...
function simLPPRRender() {
  ...
  var lignes = simLPPRGammeProduits();
  if (lignes.length === 0) {
    resEl.innerHTML = '<p style="font-size:12px;color:var(--text-ter)">Aucun produit trouvé sur ce périmètre. Vérifie que Thuasne/Podowell/Gibaud/Enovis existent, que les Gamme labo (Compression, CHUT, CHUP) sont classées, et que le LPPR est renseigné.</p>';
  ...
  var html = '<table style="border-collapse:collapse;font-size:11px;width:100%"><thead><tr style="background:#7c3aed;color:#fff">' +
    '<th style="padding:5px 8px;text-align:left">Labo</th><th style="padding:5px 8px;text-align:left">Gamme</th><th style="padding:5px 8px;text-align:left">Produit</th>' +
