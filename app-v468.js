...
// rattachés, rotation mensuelle actuelle, projection annuelle, et achats déjà réalisés cette année
// si disponibles. Affiché en tête de chaque gamme dans Cond. commerciales.
function condResumeMarcheHTML(laboId, m) {
  ...
  var projAn = rotationM * 12;
  var realiseM = achatsLabos[laboId] ? condAchatsRealiseMarche(laboId, m.label) : 0;
  var html = '<div style="margin-top:4px;margin-bottom:4px;padding:6px 8px;background:var(--accent-bg);border-radius:var(--radius-sm);font-size:11px;color:var(--text-sec);display:flex;gap:14px;flex-wrap:wrap">';
  html += '<span><strong>' + nbProduits + '</strong> produit(s) rattaché(s)</span>';
  if (rotationM > 0) {
    html += '<span>rotation actuelle : <strong>' + rotationM.toFixed(1) + ' u/mois</strong> (≈ <strong>' + Math.round(projAn) + ' u/an</strong> projeté)</span>';
  } else {
