...
function showTab(t) {
  var WIDE = ['cat','sim','cmd','prixnet','decision','cycle','simlppr','meq','map'];
  var mainEl = document.querySelector('.main');
  ['neg','prix','lex','pm','bundle','strat','verdict','mc','cond','guide','prixnet','cmd','cat','sim','workflow','decision','cycle','simlppr','meq','map'].forEach(function(x) {
    ...
    reader.onload = function (ev) {
      try {
        ...
        rows.slice(1).forEach(function (r) {
          ...
          condVentesOspharm[ean] = { qtes_n: qn, qtes_n1: qn1, ca_n: can, ca_n1: can1, stock: stk, libelle: lib, labo_lgpi: labo };
          nb++;
        });
        ...
        meqInited = false; meqInit(); meqRender();
        alert('✅ Ventes LGPI importées\n• ' + nb + ' EANs\n' +
              (joined >= 0 ? '• ' + joined + ' EANs joints avec MAP_PRODUIT_MARCHE\n• ' + Object.keys(condMarchesVirtuels).length + ' sous-marchés calculés' : '→ Importez MAP_PRODUIT_MARCHE pour calculer les marchés.'));
      } catch(err) { console.error(err); alert('Erreur : ' + err.message); }
    };
    reader.readAsArrayBuffer(file);
  };
...
function meqRenderVueLabo() {
  ...
  content.innerHTML =
    '<div class="card">' +
    '<p class="card-title">' + laboNom + ' — Position dans ' + lignes.length + ' sous-marché' + (lignes.length > 1 ? 's' : '') + '</p>' +
