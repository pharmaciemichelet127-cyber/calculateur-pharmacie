...
function catChargeLabo() {
  ...
  catProduits = src.map(function(p) {
    ...
    // Bausch + Lomb exclu du classificateur automatique par mots-cles (catalogue OTC tres heterogene,
    // les en-tetes auto-generes - MINERAUX, VITAMINES, etc. - n'ont pas de sens pour ce labo)
    var isBauschLomb = labo && labo.nom && labo.nom.toUpperCase().indexOf('BAUSCH') >= 0;
    var autoClass = isBauschLomb ? { cat:'', scat:'', marche:'' } : catClassifierProduit(p.nom || '');
    return {
      ean:          p.ean || '',
      nom:          p.nom || p.libelle || '',
      moy:          p.moy || 0,
...
async function catClassifierAuto(useLabo) {
  ...
  // Bausch + Lomb exclu (catalogue OTC heterogene, classification par mots-cles non pertinente)
  var labo = condLabos[catLaboActif];
  if (labo && labo.nom && labo.nom.toUpperCase().indexOf('BAUSCH') >= 0) {
    if (status) status.textContent = 'Auto-classification desactivee pour ce labo (Bausch + Lomb).';
    return;
  }
  catProduits.forEach(function(p) {
