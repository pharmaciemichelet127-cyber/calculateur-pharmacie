...
function condRenderMarches() {
  ...
  if (marches.length === 0) {
    container.innerHTML = engHtml + '<p style="font-size:12px;color:var(--text-ter);padding:8px 0">Aucun marché ouvert. Cliquez sur + Marché.</p>' + condGroupesLiesHTML() + kenvueBtn + chauvinBtn + podowellBtn + thuasneComprBtn;
    return;
  }
  container.innerHTML = engHtml + marches.map(function(m, i) {
    return '<div style="display:flex;flex-direction:column;gap:4px;background:var(--surface2);padding:8px 10px;border-radius:var(--radius-sm);border-left:3px solid var(--accent)">' +
      ...
      condComposerHTML(i) +
      '</div>';
  }).join('') + condGroupesLiesHTML() + kenvueBtn + chauvinBtn + podowellBtn + thuasneComprBtn;
}
...
// ===== CLASSEMENT GAMME LABO PAR ZONE ANATOMIQUE — THUASNE =====
// Renseigne la colonne "Gamme labo" (p.famille) pour TOUT le catalogue Thuasne, par mot-clé dans le
// nom du produit (ex: CHEV./MALLEO -> Cheville/Pied), avec repli sur la Famille fournisseur
// (Lingerie, Prothèses mamaires, Lymphologie...) quand elle est déjà suffisamment précise, et sur la
// Famille brute en dernier recours pour ne jamais laisser un produit sans aucune classification.
// Calibré sur le vrai catalogue Thuasne (noms/abréviations vérifiés) — couvre ~95% des références.
function condClasserThuasneGammesAnatomiques() {
  ...
  (labo.produits || []).forEach(function(p) {
    ...
    var fam = p.sous_cat || p.categorie_fournisseur || '';
    var gamme;
    if (n.indexOf('COLLIER') >= 0 || n.indexOf('CERVICAL') >= 0) gamme = 'Cervical (colliers)';
    ...
    else if (fam) gamme = fam; // repli : Lingerie/Maillots, Prothèses mamaires, Lymphologie, Vêtements de compression, Bandes, HomeCare Light, Home Care, Autres...
