...
async function catImporterXLSX(input) {
  ...
  try {
    ...
    for (var ri = 0; ri < Math.min(rows.length, 20); ri++) {
      ...
      var hasGamme = rowUp.some(function(c){
        return c.indexOf('GAMME') >= 0 || c.indexOf('DESIGNATION') >= 0 || c.indexOf('LIBELLE') >= 0 ||
          c.indexOf('REFERENCE') >= 0 || c === 'PRODUIT' || c.indexOf('PRODUITS') >= 0;
      });
    ...
    if (idxTarif < 0) idxTarif = headerUp.findIndex(function(c){ return c.indexOf('PTHT') >= 0 || (c.indexOf('PT') >= 0 && c.indexOf('HT') >= 0 && c.indexOf('PPC') < 0 && c.indexOf('TTC') < 0); });
