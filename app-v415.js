...
async function decImporterStockLGPI(input) {
  ...
  if (status) { status.textContent = 'Lecture PDF LGPI (stock)...'; status.style.color = 'var(--text-sec)'; }
  ...
  try {
    var resp = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        messages: [{
          ...
          content: [
            { type:'document', source:{type:'base64', media_type:'application/pdf', data:b64} },
            { type:'text', text: 'Pour chaque ligne produit de cet etat LGPI, extrais uniquement : ean (13 chiffres ou vide) et stock (entier, colonne Stock). Ignore les sous-totaux. UNIQUEMENT JSON sans texte ni markdown. Format strict : {"produits":[{"ean":"...","stock":0}]}' }
          ]
        }]
      })
    });
    ...
    var raw = (data.content||[]).map(function(c){return c.text||'';}).join('').trim();
    var jsonMatch = raw.match(/\{[\s\S]*\}/);
    if (!jsonMatch) { if (status) { status.textContent = 'Aucune donnee extraite.'; status.style.color = 'var(--danger)'; } return; }
    var parsed = JSON.parse(jsonMatch[0]);
    var stocks = parsed.produits || [];
    var stockByEan = {};
    stocks.forEach(function(s){ if (s.ean) stockByEan[String(s.ean)] = s.stock || 0; });

    var maj = 0;
    decOspharmProds[labo].forEach(function(p) {
      if (p.ean && stockByEan[String(p.ean)] !== undefined) {
        p.stock = stockByEan[String(p.ean)];
        maj++;
      }
    });

    if (status) { status.textContent = '✓ Stock LGPI mis a jour pour ' + maj + ' produit(s) (plus fiable que le stock Ospharm).'; status.style.color = 'var(--accent-text)'; }
...
function decRenderConditionsCommerciales(labo) {
  ...
  var laboObj = null;
  Object.keys(condLabos).forEach(function(id) {
    if (condLabos[id].nom && labo && condLabos[id].nom.toUpperCase().indexOf(labo.toUpperCase()) >= 0) laboObj = condLabos[id];
  });
...
function decAnalysePalier() {
  ...
  var laboObj = null;
  Object.keys(condLabos).forEach(function(id) {
    if (condLabos[id].nom && laboNom && condLabos[id].nom.toUpperCase().indexOf(laboNom.toUpperCase()) >= 0) laboObj = condLabos[id];
  });
