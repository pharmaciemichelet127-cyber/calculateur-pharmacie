        ...
        if (m) {
          ...
          (parsed.produits||[]).forEach(function(r) {
            ...
            if (p) {
              ...
              // Chercher par label exact, puis par label partiel, puis par id
              var marcheDef = marches.find(function(m){ return m.label === mid; })
                           || marches.find(function(m){ return mid && m.label && m.label.toLowerCase().indexOf(mid.toLowerCase()) >= 0; })
                           || marches.find(function(m){ return m.id === mid; });
              if (marcheDef) {
