...
var catRowLimit = 300;
function catAfficherTousProduits() {
  catRowLimit = Infinity;
  catRenderTable();
}
...
function catRenderTable() {
  ...
  var rowsRendered = 0;
  var tronque = totalProduits > catRowLimit;
  if (tronque) {
    html += '<tr><td colspan="' + (showLPPR ? 11 : 10) + '" style="padding:8px 10px;background:#fffbeb;border-bottom:1px solid #f59e0b;font-size:11px;color:#92400e">' +
      '⚠ Catalogue volumineux (' + totalProduits.toLocaleString('fr') + ' produits) — affichage limité aux ' + catRowLimit + ' premières lignes pour éviter de ralentir le navigateur. ' +
      '<button type="button" onclick="catAfficherTousProduits()" style="margin-left:8px;font-size:11px;padding:3px 10px;background:#f59e0b;color:#fff;border:none;border-radius:3px;cursor:pointer;font-weight:600">Tout afficher (peut être lent)</button>' +
      ' — utilise plutôt la recherche ou un filtre par marché pour les gros catalogues.</td></tr>';
  }
  ...
  sortedKeys.forEach(function(marcheId) {
    ...
    items.forEach(function(item) {
      if (rowsRendered >= catRowLimit) return;
      rowsRendered++;
