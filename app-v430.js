...
function decAffecterMarches(labo, marches, prodsOverride, eanToMarche) {
  ...
  for (var i = 0; i < prods.length; i++) {
    ...
    if (idx < 0) continue; // -2 = delasse au catalogue ; -1 = EAN inconnu sans marche fourre-tout designe -> hors marche
  ...
  if (marches.length > 0) {
    ...
    if (prodsLabo.length > 0) {
      ...
      prodsLabo.forEach(function(p) {
        ...
        else if (found === -2) horsMarche.push(p);
        else if (idxDefM >= 0) groupes[idxDefM].push(p);
        else horsMarche.push(p);
      });
