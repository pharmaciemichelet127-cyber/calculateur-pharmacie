...
function catReporterMarcheVersGammeLabo() {
  ...
  var nbReportes = 0, nbIgnores = 0;
  catProduits.forEach(function(p) {
    // Le label (p.marche) n'est synchronisé que si l'affectation a été faite via le menu déroulant
    // (catUpdateProduit). Beaucoup de produits arrivent par import XLSX/composeur avec seulement
    // p.marche_id rempli : on résout alors le label depuis la liste des marchés du labo.
    var marcheVal = p.marche || '';
    if (!marcheVal && p.marche_id && p.marche_id !== 'HORS_MARCHE') {
      var mDef = marchesActifs.find(function(m){ return m.id === p.marche_id; });
      marcheVal = mDef ? mDef.label : p.marche_id;
    }
    if (!marcheVal || marcheVal === 'HORS_MARCHE' || p.marche_id === 'HORS_MARCHE') { nbIgnores++; return; }
    p.famille = marcheVal;
    nbReportes++;
  });
  catRenderTable();
  catSauvegarder();
  alert('Gamme labo mise à jour : ' + nbReportes + ' produit(s) reporté(s) depuis le Marché. ' + nbIgnores + ' produit(s) non affecté(s) ou Hors marché laissé(s) tel(s) quel(s).');
}
