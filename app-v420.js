...
function decRenderConditionsCommerciales(labo) {
  ...
  if (marches.length > 0) {
    ...
    if (prodsLabo.length > 0) {
      ...
      html += '<thead><tr><th style="' + thS2 + ';text-align:left">Produit</th><th style="' + thS2 + '">CA HT</th><th style="' + thS2 + '">Marge</th><th style="' + thS2 + '">Taux</th></tr></thead><tbody>';
      marches.forEach(function(m, mi) {
        ...
        html += '<tr><td colspan="4" style="padding:6px 8px;font-size:10px;font-weight:700;color:var(--accent-text);background:var(--surface2);text-align:left">' + m.label + ' (' + (m.rem||0) + '%) — ' + grp.length + ' réf. · ' + Math.round(sousCa).toLocaleString('fr') + ' €</td></tr>';
      ...
      if (horsMarche.length > 0) {
        ...
        html += '<tr><td colspan="4" style="padding:6px 8px;font-size:10px;font-weight:700;color:var(--text-ter);background:var(--surface2);text-align:left">Hors marché négocié — ' + horsMarche.length + ' réf. · ' + Math.round(hCa).toLocaleString('fr') + ' €</td></tr>';
      }
      html += '</tbody></table></div>';
    }
  }
