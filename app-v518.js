  ...
  input.onchange = function (e) {
    ...
    reader.onload = function (ev) {
      try {
        ...
        meqInited = false; meqInit(); meqRender();
        mapInited = true; if (typeof mapRender === 'function') mapRender();
...
// Filtre + limite résultats
function mapGetFiltered(q) {
  q = (q || '').toLowerCase().trim();
  var keys = Object.keys(condMapProduitMarche);
  if (!q) return { entries: keys.slice(0, 60), total: keys.length, filtered: keys.length };
  ...
  return { entries: filtered.slice(0, 200), total: keys.length, filtered: filtered.length };
}
...
function mapRender() {
  ...
  var subtitle = q
    ? res.filtered + ' résultat' + (res.filtered>1?'s':'') + ' sur ' + total + ' mappings'
    : (res.entries.length < total ? '60 premiers sur ' + total + ' mappings — affinez la recherche' : total + ' mappings');
