...
async function catClassifierAuto(useLabo) {
  ...
  // Claude API pour tous les produits sans marché (v3.89)
            ...
            messages: [{role:'user', content:
              (function(){
                ...
                var marchesStr = marchesDef.length > 0
                  ? 'Marches negocies disponibles (utilise EXACTEMENT ces labels pour marche): ' + marchesDef.map(function(m){ return m.label + ' (' + m.rem + '%)'; }).join(', ')
                  : 'gamme produit ex: PHOTODERM, CICALFATE, CLEANANCE, XERACALM, HYDRANCE';
                return 'Classifie ces produits pharmaceutiques/dermo-cosmetiques. Pour chacun: cat (Soin corps, Soin visage, Solaire, Capillaire, Complement alimentaire, Hygiene...), scat (sous-categorie courte), marche (' + marchesStr + '). Assigne toujours un marche si possible. JSON UNIQUEMENT: {"produits":[{"idx":1,"cat":"...","scat":"...","marche":"..."}]}\n\n' + noms;
              })()
            }]
          })
        });
