...
function decAnalysePalier() {
  ...
  if (decCondCurrent && decCondCurrent.seuilCA && decCurrentStats.caht) {
    var caActuel = decCurrentStats.caht;
...
function decRenderPalierAnalyse() {
  ...
  analyses.forEach(function(a) {
    if (a.type === 'references') {
      ...
      html += icone2 + ' CA actuel : <strong>' + Math.round(a.valeurActuelle).toLocaleString('fr') + ' €</strong> → Seuil : <strong>' + Math.round(a.suivant.seuil).toLocaleString('fr') + ' €</strong>';
      html += ' — écart : <strong>' + a.ecartPct.toFixed(1) + '%</strong>';
