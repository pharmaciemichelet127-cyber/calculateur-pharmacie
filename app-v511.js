...
function meqInit() {
  ...
  if (!sel) return;
  var data = meqGetData();
  sel.innerHTML = data.map(function (sm) {
    return '<option value="' + sm.id + '">' + sm.libelle + '</option>';
  }).join('');
}
  ...
  if (Object.keys(condMarchesVirtuels).length) {
    return Object.keys(condMarchesVirtuels)
      .map(function (k) {
        ...
        return { id: k, libelle: sm.libelle || MEQ_LIBELLES[k] || k, segment: sm.segment || 'SOCLE', produits: sm.produits || [], totalN: totalN };
      })
