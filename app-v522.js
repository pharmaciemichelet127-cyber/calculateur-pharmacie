...
function engRender() {
  ...
  var sms = Object.keys(condMarchesVirtuels).filter(function(smId) {
    var sm = condMarchesVirtuels[smId];
    return sm.produits && sm.produits.some(function(p){ return p.labo_nom === laboNom; });
  });

  if (!sms.length) {
    div.innerHTML = '<div class="card"><p style="color:var(--text-ter);font-size:12px">Aucun sous-marché trouvé pour ce labo. Vérifiez que les ventes LGPI sont importées.</p></div>';
    return;
  }
  ...
  var rows = sms.map(function(smId) {
    ...
    var smLib   = sm ? (sm.libelle || smId) : smId;
    ...
    return '<tr style="border-bottom:1px solid var(--border);vertical-align:top">' +
      ...
      '<td style="padding:8px 6px">' +
        ...
        '<div style="font-size:10px;color:var(--text-ter)">' + smId + ' · ' + sm.marche + '</div>' +
        '<div style="margin-top:3px">' + statutBadge + '</div>' +
      '</td>' +
      // Volumes
      '<td style="padding:8px 6px;font-size:11px;text-align:right">' +
        '<div><b>' + qnLabo + '</b> u. N</div>' +
        '<div style="color:var(--text-sec)">' + qn1Labo + ' u. N-1</div>' +
        '<div style="color:var(--text-sec)">Marché: ' + totalN + 'u.</div>' +
      '</td>' +
      // PDM
