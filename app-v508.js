...
function meqGetData() {
  // Point d'extension : si des marches virtuels reels ont ete importes
  // (condMarchesVirtuels), les utiliser en priorite ; sinon donnees test.
  if (typeof condMarchesVirtuels === 'object' && condMarchesVirtuels && Object.keys(condMarchesVirtuels).length) {
    return Object.keys(condMarchesVirtuels).map(function (k) {
      var sm = condMarchesVirtuels[k];
      return { id: k, libelle: sm.libelle || k, segment: sm.segment || '', produits: sm.produits || [] };
    });
  }
  return meqDataTest;
}
...
function meqRender() {
  ...
  var rows = sm.produits.map(function (p, i) {
    ...
    var condHtml = cond
      ...
      : '<span style="color:var(--text-ter)">Labo non trouvé dans Conditions commerciales</span>';
    return '<tr style="' + (estSelectionne ? 'background:var(--accent-bg)' : '') + '">' +
      '<td style="padding:6px 8px;font-weight:600">' + p.labo_nom + '</td>' +
      '<td style="padding:6px 8px;font-size:11px;color:var(--text-sec)">' + (p.designation || '') + '<br><span style="font-size:10px;color:var(--text-ter)">EAN ' + p.ean + '</span></td>' +
  ...
  content.innerHTML =
    '<div class="card">' +
    '<p class="card-title">' + sm.libelle + '</p>' +
